...
public class WaterandLavaVariables {
	public static class MapVariables extends WorldSavedData {
		public static final String DATA_NAME = "obsidian_mapvars";
		public boolean OverlayElecro = false;
		public boolean ActivePotion1 = false;
		...
		public void readFromNBT(NBTTagCompound nbt) {
			OverlayElecro = nbt.getBoolean("OverlayElecro");
			ActivePotion1 = nbt.getBoolean("ActivePotion1");
			Thunder = nbt.getBoolean("Thunder");
		}

		@Override
		public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
			nbt.setBoolean("OverlayElecro", OverlayElecro);
			nbt.setBoolean("ActivePotion1", ActivePotion1);
			nbt.setBoolean("Thunder", Thunder);
			return nbt;
		}
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				WaterandLava.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(0, this));
			} else {
				WaterandLava.PACKET_HANDLER.sendToAll(new WorldSavedDataSyncMessage(0, this));
			}
		}
	...
	public static class WorldVariables extends WorldSavedData {
		public static final String DATA_NAME = "obsidian_worldvars";
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				WaterandLava.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(1, this));
			} else {
				WaterandLava.PACKET_HANDLER.sendToDimension(new WorldSavedDataSyncMessage(1, this), world.provider.getDimension());
			}
		}
	...
	public static class WorldSavedDataSyncMessageHandler implements IMessageHandler<WorldSavedDataSyncMessage, IMessage> {
		...
		private void syncData(WorldSavedDataSyncMessage message, MessageContext context, World world) {
			if (context.side == Side.SERVER) {
				...
				if (message.type == 0)
					WaterandLava.PACKET_HANDLER.sendToAll(message);
				else
					WaterandLava.PACKET_HANDLER.sendToDimension(message, world.provider.getDimension());
			}
			if (message.type == 0) {
