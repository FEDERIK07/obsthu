...
import net.mcreator.obsidian.block.BlockElettrifiedLog;
import net.mcreator.obsidian.block.BlockElettrifiedLeavs;
import net.mcreator.obsidian.ElementsWaterandLava;

import java.util.Random;

@ElementsWaterandLava.ModElement.Tag
public class BiomeThunderZone extends ElementsWaterandLava.ModElement {
	@GameRegistry.ObjectHolder("obsidian:thunderzone")
	public static final BiomeGenCustom biome = null;
	public BiomeThunderZone(ElementsWaterandLava instance) {
		super(instance, 104);
	}
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Thunder Zone").setRainfall(1F).setBaseHeight(0.1F).setWaterColor(-6684724).setHeightVariation(0.2F)
					.setTemperature(0F));
			setRegistryName("thunderzone");
	...
	static class CustomTree extends WorldGenAbstractTree {
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												...
												|| state.getBlock() == BlockElettrifiedLeavs.block.getDefaultState().getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, BlockElettrifiedLeavs.block.getDefaultState());
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.VINE.getDefaultState().getBlock()
									|| state.getBlock() == BlockElettrifiedLeavs.block.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), BlockElettrifiedLog.block.getDefaultState());
							}
						}
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BlockElettrifiedLog.block.getDefaultState().getBlock()
					|| blockType == BlockElettrifiedLeavs.block.getDefaultState().getBlock() || blockType == Blocks.GRASS.getDefaultState().getBlock()
